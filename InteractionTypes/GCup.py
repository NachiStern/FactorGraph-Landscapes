...
from numpy import eye, sqrt, dot, shape, array, empty_like, zeros, log, outer, empty, argsort
...
def ConnectStablyByDistance(Objs, maxDist, params = [], Metric = [], smallR=0):
    ...
    return Interactions, Adjacency, Z


def ConnectRandomZ(Objs, Znum=1, params = [], Metric = [], smallR=0):
    Dim = Objs[0].Parameters['SpatialDimension']
    lo = len(Objs)

    assert type(Znum) == int
    ...
    Dists = zeros([lo,lo])
    for i in range(lo):
        P[i] = Objs[i].Variables['Position']
    for i in range(lo):
        for j in range(lo):
            P1 = P[i]
            P2 = P[j]
            Diff = P2 - P1
            Dists[i,j] = sqrt(dot(dot(Metric, Diff), Diff))
